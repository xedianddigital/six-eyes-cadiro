 ...
 * notice on their own.
 */
const RELEASES_API_URL = 'https://api.github.com/repos/xedianddigital/six-eyes-cadiro/releases/latest'
const RELEASES_PAGE_URL = 'https://github.com/xedianddigital/six-eyes-cadiro/releases/latest'
...
function fetchLatestRelease() {
  return new Promise((resolve) => {
    const req = https.get(
      RELEASES_API_URL,
      { headers: { 'User-Agent': 'SixEyesCadiro', Accept: 'application/vnd.github+json' } },
      (res) => {
        let data = ''
        res.on('data', (c) => (data += c))
        res.on('end', () => {
...
async function runUninstall() {
  ...
  if (process.platform !== 'win32') {
    await dialog.showMessageBox({
      ...
      detail:
        'This build is a portable AppImage - delete the .AppImage file to remove it. Settings live in ~/.config/six-eyes-cadiro.',
    })
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Project page',
          click: () => shell.openExternal('https://github.com/xedianddigital/six-eyes-cadiro'),
        },
        {
